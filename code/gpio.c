 ...
 */
 ...
 void pushbutton_task(void *p)
 {
     ...
     while(1){
         ...
         xil_printf("BUTTON: ");
         for (int i = 3; 0 <= i; i--) xil_printf("%d", (button_val >> i) & 1);
         xil_printf("\r\n");
 ...
 //            }
         ...
         } else if ((button_val & 0x1 || button_val & 0x2) && count++ == 3) {
             while (xQueueSend(emergency_queue, &button_val, 0) != pdPASS) vTaskDelay(DELAY_50_MS);
         }
         vTaskDelay(DELAY_50_MS);
     }
 }
 ...
 void led_task(void *p)
 {
     ...
     /* --------------------------------------------------*/
         ...
         xQueueReceive(led_queue, &step_mode, 0);
